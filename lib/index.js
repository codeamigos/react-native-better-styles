...
const multiplyStylesValues = (styles, multiplicators) => {
    const resultStyles = {};
    Object.keys(styles).map(key => {
        Object.keys(multiplicators).map(prefix => {
            const multiplicatorValue = multiplicators[prefix];
...
const multiplyToRem = (remValue, multiplicators) => {
    const multiplied = {};
    Object.keys(multiplicators).map(prefix => {
        const multiplicatorValue = multiplicators[prefix];
...
const generatePalette = (colors) => {
    ...
    Object.keys(colors).map(name => {
        ...
        for (let i = 5; i < 100; i += 5) {
            const rgbString = Color(color).alpha(i / 100).rgb().string();
...
exports.buildStyles = {
    ...
    build: (defaultOptions = {}, callback = () => { }) => {
        ...
        const multiplicators = defaultOptions.multiplicators || exports.defaultMultiplicators;
        ...
        const fonts = defaultOptions.palette || exports.defaultPalette;
        ...
        _.assign(exports.buildStyles.sizes, multiplyToRem(remSize, multiplicators));
        _.assign(exports.buildStyles.s, RN.StyleSheet.create(Object.assign({}, multiplyStylesValues(genericPointStyles, multiplicators), multiplyStylesValues(genericRemStyles, multiplyToRem(remSize, multiplicators)), multiplyStylesValues(textRemStyles, multiplyToRem(remSize, multiplicators)), multiplyStylesValues({ f: 'fontSize' }, multiplyToRem(remSize, headings)), generateColorsPalette(palette), generateTextColorsPalette(palette), generateFonts(fonts), generateFontWeights(fontWeights), generateOpacity(), genericStaticStyles, textStaticStyles)));
        callback();
    }
};
exports.defaultMultiplicators = {
    '0': 0,
    '025': 0.25,
    '05': 0.5,
    '075': 0.75,
